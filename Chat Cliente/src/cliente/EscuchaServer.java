...
import intefaces.MiChat;
import intefaces.VentanaContactos;
			...
			while (true) {
				...
				case Comando.CHATALL:
					...
					if (!cliente.getChatsActivos().containsKey("Sala")) {
						...
						VentanaContactos.setTextoChatGeneral(cliente.getPaqueteMensaje().getUserEmisor(),cliente.getPaqueteMensaje().getMensaje());
					}
					break;
				}
			}
		...
		synchronized (cliente) {
			try {
				...
				if (cliente.getPaqueteUsuario().getListaDeConectados() != null) {
					...
					VentanaContactos.setCantUsuariosCon(modelo.getSize());
